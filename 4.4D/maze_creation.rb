...
class GameWindow < Gosu::Window
  ...
  def cell_show(array)
    ...
    while (column_index < x_cell_count)
      row_index = 0
      while (row_index < y_cell_count)
       puts "Cell x: #{column_index}, y: #{row_index} north: #{cell_check(array[column_index][row_index].north).to_s} south: #{cell_check(array[column_index][row_index].south).to_s} east: #{cell_check(array[column_index][row_index].east).to_s} west: #{cell_check(array[column_index][row_index].west).to_s}"
        row_index += 1
      end
      puts "---------------- End of Column ------------------"
      column_index += 1
