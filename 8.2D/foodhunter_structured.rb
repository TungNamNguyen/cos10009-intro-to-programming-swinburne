...
class Food
  ...
  attr_accessor :x, :y, :type, :image, :vel_x, :vel_y, :angle, :x, :y, :score, :image_main, :change, :time

  def initialize(image, type)
    @type = type
    @image_main = Gosu::Image.new(image)
    @image=@image_main
    @vel_x = rand(-2 .. 2)  # rand(1.2 .. 2.0)
    @vel_y = rand(-2 .. 2)
    @angle = 0.0
    @image_smoke= Gosu::Image.new('media/smoke.png')
    @change
    @time
...
def change_direction food
  food.vel_x = rand(-2 .. 2)  # rand(1.2 .. 2.0)
  food.vel_y = rand(-2 .. 2)
